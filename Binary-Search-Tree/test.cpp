...
bool BinarySearchTreeTest::test3() {
    ...
    // Insert elements to create a linear list formation: 1 -> 2 -> 3
    ...
    // Compare the tree's representation to the expected tree.
    string expected_level_order = "1 2 3";
    string tree_level_order = breadthFirstTraversal(bst.root_);
    ASSERT_TRUE(tree_level_order.compare(expected_level_order) == 0);

    // Check that size is expected (should be 3).
    ASSERT_TRUE(bst.size() == 3);

    // Try to remove an element that is present in the tree (e.g., 2).
    ASSERT_TRUE(bst.remove(2));

    // Compare the tree's representation after removal to the expected tree.
    expected_level_order = "1 3";
    tree_level_order = breadthFirstTraversal(bst.root_);
    ASSERT_TRUE(tree_level_order.compare(expected_level_order) == 0);

    // Check that size is updated after removal (should be 2).
    ASSERT_TRUE(bst.size() == 2);

    // Check to see if the removed element (2) is no longer in the tree.
    ASSERT_FALSE(bst.exists(2));

    // Try to remove an element that is not present in the tree (e.g., 4).
    ASSERT_FALSE(bst.remove(4));
...
bool BinarySearchTreeTest::test4() {
    ...
    // Compare the tree's representation to the expected tree.
    string expected_level_order = "5 3 8 2 4";
    string tree_level_order = breadthFirstTraversal(bst.root_);
    ASSERT_TRUE(tree_level_order.compare(expected_level_order) == 0);

    // Check that size is expected (should be 5).
    ASSERT_TRUE(bst.size() == 5);

    // Try to remove a node with one child (e.g., node with value 3).
    ASSERT_TRUE(bst.remove(3));

    // Compare the tree's representation after removal to the expected tree.
    bst.print();
    expected_level_order = "5 2 8 4";
    tree_level_order = breadthFirstTraversal(bst.root_);
    ASSERT_TRUE(tree_level_order.compare(expected_level_order) == 0);

    // Check that size is updated after removal (should be 4).
    ASSERT_TRUE(bst.size() == 4);

    // Check to see if the removed node (with value 3) is no longer in the tree.
    ASSERT_FALSE(bst.exists(3));

    // Try to remove a node that is not present in the tree (e.g., node with value 6).
...
bool BinarySearchTreeTest::test5() {
    ...
    // Compare the tree's representation to the expected tree.
    string expected_level_order = "5 3 8 2 4 7 9";
    string tree_level_order = breadthFirstTraversal(bst.root_);
    ASSERT_TRUE(tree_level_order.compare(expected_level_order) == 0);

    // Check that size is expected (should be 7).
    ASSERT_TRUE(bst.size() == 7);

    // Remove nodes one by one until nothing remains in the tree.
    while (bst.size() > 0) {
        BinarySearchTree::DataType max_val = bst.max();
        ASSERT_TRUE(bst.exists(max_val));
        ASSERT_TRUE(bst.remove(max_val));
    }

    // After removing all nodes, the tree should be empty (root should be nullptr).
    ASSERT_TRUE(bst.getRootNode() == nullptr);
...
bool BinarySearchTreeTest::test8() {
    ...
    BinarySearchTree::DataType in[10] = {8, 3, 10, 1, 6, 9, 15, 2, 5, 12};
    for (auto val : in) {
        ASSERT_TRUE(bst.insert(val));
    }

    // Check the resulting tree after inserts.
    string expected_tree = "8 3 10 1 6 9 15 2 5 12";
    string tree_level_order = breadthFirstTraversal(bst.root_);
    ASSERT_TRUE(tree_level_order.compare(expected_tree) == 0);
    ...
    // Check the resulting tree after removes.
    expected_tree = "8 2 12 1 5 9 15";
    tree_level_order = breadthFirstTraversal(bst.root_);
    ASSERT_TRUE(tree_level_order.compare(expected_tree) == 0);

    // Insert more nodes into the tree.
    BinarySearchTree::DataType additional_in[4] = {4, 7, 11, 13};
    for (auto val : additional_in) {
        ASSERT_TRUE(bst.insert(val));
    }

    // Check the resulting tree after more inserts.
    expected_tree = "8 2 12 1 5 9 15 4 7 11 13";
    tree_level_order = breadthFirstTraversal(bst.root_);
    ASSERT_TRUE(tree_level_order.compare(expected_tree) == 0);
